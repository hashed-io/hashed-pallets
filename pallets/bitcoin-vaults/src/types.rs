...
impl<T: Config> Proposal<T> {
	...
	// 	self.status.eq(&ProposalStatus::ReadyToBroadcast) &&
	// self.offchain_status.eq(&BDKStatus::Valid) }
}
...
impl ProposalStatus {
	...
	pub fn next_status(&self) -> Self {
		...
		match *self {
			...
			ReadyToFinalize(false) => Finalized, /* it will be finalized but the broadcast is */
			// still pending
			ReadyToFinalize(true) => Broadcasted, /* the "true" flag value will finalize and */
			// broadcast it
