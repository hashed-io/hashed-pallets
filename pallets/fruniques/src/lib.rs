...
pub mod pallet {
		...
		T: pallet_uniques::Config<CollectionId = CollectionId, ItemId = ItemId>,
	{
		...
		/// ### Considerations:
		/// This functions enables the owner of a collection to invite a user to become a
		...
		/// - `origin` must be signed by the Root origin.
		/// - `class_id` must be a valid class of the asset class.
		/// - `witness` the witness data to destroy the collection. This is used to prevent
		///   accidental destruction of the collection. The witness data is retrieved from the
		///   `class` storage.
		/// - `maybe_check_owner` Optional value to check if the owner of the collection is the same
		///   as the signer.
		/// ### Considerations:
		/// This function is only used for testing purposes. Or in case someone calls uniques pallet
		/// directly. This function it's not expected to be used in production as it can lead to
