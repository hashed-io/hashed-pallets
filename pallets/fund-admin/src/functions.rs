...
impl<T: Config> Pallet<T> {
	...
	fn do_unassign_user(
		...
		<UsersByProject<T>>::try_mutate_exists::<_, _, DispatchError, _>(
			project_id,
			|users_option| {
	...
	fn do_delete_user(user: T::AccountId) -> DispatchResult {
		...
		match user_data.role {
			...
			_ => {
				...
				// Remove user from ProjectsByUser storagemap. No longer required, the admnistator
				// first needs to unassign the user from all its projects.

				// Remove user from UsersByProject storagemap. No longer required, the admnistator
				// first needs to unassign the user from all its projects.
	...
	pub fn do_execute_transactions(
		...
		// Ensure project exists & is not completed so helper private functions doesn't need to
		// check it again
		Self::is_project_completed(project_id)?;
	...
	fn check_user_role(user: T::AccountId, role: ProxyRole) -> DispatchResult {
		...
		match user_data.role {
			...
			// Builders, Issuers & Regional Centers don't have a limit on how many projects they can
			// be assigned to
			_ => Ok(()),
		}
	}
	...
	fn do_calculate_drawdown_total_amount(
		...
		if !TransactionsByDrawdown::<T>::get(project_id, drawdown_id).is_empty() {
			...
			let transactions_by_drawdown =
				TransactionsByDrawdown::<T>::get(project_id, drawdown_id);
	...
	fn do_delete_expenditure_transactions(expenditure_id: ExpenditureId) -> DispatchResult {
		...
		for drawdown_id in drawdowns.iter().cloned() {
			...
			// zero
			if !<TransactionsByDrawdown<T>>::get(expenditure_data.project_id, drawdown_id)
				.is_empty()
			{
						...
						.cloned()
				{
					...
					if transaction_data.expenditure_id == expenditure_id {
						...
						<TransactionsByDrawdown<T>>::try_mutate_exists::<_, _, _, DispatchError, _>(
							...
							drawdown_id,
							|transactions_option| {
								let transactions = transactions_option
									.as_mut()
									.ok_or(Error::<T>::DrawdownHasNoTransactions)?;
								transactions.retain(|transaction| transaction != &transaction_id);
								if transactions.is_empty() {
									transactions_option.clone_from(&None);
								}
	...
	pub fn do_recovery_drawdown(
		...
		// If the administrator adds more transactions to the given drawdown, update the added
		// transaction to the drawdown's transactions status
		// Get drawdown transactions
		if !<TransactionsByDrawdown<T>>::get(project_id, drawdown_id).is_empty() {
			// If a transaction is in a diffferent status than Approved or Confirmed, set it to the
	...
	pub fn do_recovery_revenue(
		...
		// If the administrator adds more transactions to the given revenue, update the added
		// transaction to the revenue's transactions status
		// Get revenue transactions
		if !<TransactionsByRevenue<T>>::get(project_id, revenue_id).is_empty() {
			// If a transaction is in a diffferent status than Approved, set it to the current
			// revenue status
			for transaction_id in
