...
pub mod pallet {
	...
	pub(super) type TransactionsByRevenue<T: Config> = StorageDoubleMap<
		...
		BoundedVec<RevenueTransactionId, T::MaxTransactionsPerRevenue>, /* Value revenue
		            * transactions */
		ValueQuery,
	>;
	...
	pub enum Error<T> {
		...
		/// Can not insert (CUDAction: Create) bank confmirng documents if the drawdown has already
		/// bank confirming documents
	...
	impl<T: Config> Pallet<T> {
		...
		/// - `drawdown_id`: The ID of the drawdown from which the recovery drawdown will be
		///   executed
		/// - `transactions`: The list of transactions that will be executed in the recovery
		///   drawdown
		///
		/// # Errors
		///
