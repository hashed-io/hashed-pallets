...
fn users_update_a_registered_account_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(FundAdmin::users(
			...
			make_user(
				2,
				Some(make_field_name("Alice Regional Center Updated")),
				None,
				CUDAction::Update
			)
		));
...
fn projects_assign_an_investor_to_a_project_works() {
	new_test_ext().execute_with(|| {
		...
		let investor_assignment =
			make_user_assignation(3, ProxyRole::Investor, AssignAction::Assign);
...
fn projects_unassign_a_builder_from_a_project_works() {
	new_test_ext().execute_with(|| {
		...
		let builder_unassignment =
			make_user_assignation(2, ProxyRole::Builder, AssignAction::Unassign);
...
fn projects_unassign_an_issuer_from_a_project_works() {
	new_test_ext().execute_with(|| {
		...
		let issuer_unassignment =
			make_user_assignation(4, ProxyRole::Issuer, AssignAction::Unassign);
...
fn user_cannot_be_assigned_to_a_project_with_a_different_role_should_fail() {
	new_test_ext().execute_with(|| {
		...
		let investor_assignment =
			make_user_assignation(2, ProxyRole::Investor, AssignAction::Assign);
...
fn expenditures_an_admin_can_delete_an_expenditure_containing_transactions_with_zero_amount_works()
{
	new_test_ext().execute_with(|| {
		...
		let transaction_data =
			make_transaction(Some(expenditure_id), Some(0), CUDAction::Create, None);
...
fn expenditures_an_admin_cannot_delete_a_expenditure_that_is_in_use_submitted_status_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			DrawdownsInfo::<Test>::get(drawdown_id).unwrap().status,
...
fn job_eligibles_deleting_a_job_eligible_requires_a_job_eligible_id_should_fail() {
	new_test_ext().execute_with(|| {
		...
		let del_job_eligible_data =
			make_job_eligible(None, None, None, None, CUDAction::Delete, None);
...
fn drawdowns_a_user_cannot_create_a_transaction_without_an_amount_should_fail() {
	new_test_ext().execute_with(|| {
		...
		let transaction_data =
			make_transaction(Some(expenditure_id), None, CUDAction::Create, None);
...
fn drawdowns_after_a_drawdown_is_approved_the_next_one_is_generated_autoamtically_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			DrawdownsInfo::<Test>::get(next_drawdown_id).unwrap().status,
			DrawdownStatus::Draft
		);
	});
}
...
fn revenues_a_builder_saves_a_revenue_as_draft_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			RevenueTransactionsInfo::<Test>::get(revenue_transaction_id).unwrap().amount,
			10000
		);
		assert_eq!(
...
fn revenues_an_administrator_rejects_a_given_revenue_works() {
	new_test_ext().execute_with(|| {
		...
		let transaction_feedback =
			make_transaction_feedback(revenue_transaction_id, feedback.clone());
...
fn inflation_rate_an_administrator_can_set_the_inflation_rate_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			ProjectsInfo::<Test>::get(project_id).unwrap().inflation_rate,
			Some(inflation_rate)
		);
	});
}
...
fn an_administrators_adds_a_transaction_for_an_approved_revenue_works() {
	new_test_ext().execute_with(|| {
		...
		let transaction_id_2 =
			get_revenue_transaction_id(project_id, revenue_id, job_eligible_id_2);
