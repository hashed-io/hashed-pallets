...
impl<T: Config> Pallet<T> {
	...
	// MarketplaceRole) -> bool{     let roles = match
	// <MarketplacesByAuthority<T>>::try_get(account, marketplace_id){         Ok(roles) => roles,
	//         Err(_) => return false,
	//     };
	...
	fn can_this_item_receive_sell_orders(
		...
		//if len is == 0, it means that there is no offers for this item, maybe it's the first
		// entry
		if offers.len() > 0 {
			for offer in offers {
	...
	fn _delete_all_sell_orders_for_this_item(
		...
		for offer_id in offers_ids {
			...
			//ensure the offer_type is SellOrder, because this vector also contains offers of
			// BuyOrder OfferType.
