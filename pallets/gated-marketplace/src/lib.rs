...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		/// An applicant was accepted or rejected on the marketplace. [AccountOrApplication,
		/// market_id, status]
		...
		T: pallet_uniques::Config<CollectionId = u32, ItemId = u32>,
	{
		...
		///
		/// ### Considerations:
		/// - Once a user is blocked, the user won't be able to join the marketplace until
		///   unblocked.
		#[pallet::call_index(2)]
		#[pallet::weight(Weight::from_parts(10_000,0) + T::DbWeight::get().writes(1))]
		pub fn block_user(
		...
		///
		/// ### Considerations:
		/// - Since this is a second chance, you can replace your previous documents, up to the
		///   maximum allowed (10).
		/// - The custodian account is optional. You can replace the previous custodian.
		/// - Since we know the application exists, we can check the current status of the
		///   application.
		#[pallet::call_index(4)]
		#[pallet::weight(Weight::from_parts(10_000,0) + T::DbWeight::get().writes(1))]
		pub fn reapply(
		...
		///
		/// ### Considerations:
		/// - You can only accept or reject applications where you are the owner/admin of the
		///   marketplace.
		...
		/// - Once the sell order is accepted, the ownership of the item is transferred to the
		///   buyer.
