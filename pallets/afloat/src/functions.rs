...
impl<T: Config> Pallet<T> {
	...
	pub fn do_create_afloat_frunique(origin: OriginFor<T>, admin: T::AccountId) -> DispatchResult
	where
	...
	/// - `phone`: An optional `Option<ShortString>` containing the user's phone number, or None if
	...
	/// # Errors
	///
	/// Returns an `Error` if the requested user account is not found or if the edit operation
	/// fails.
	///
	/// # Returns
	///
	...
	/// # Side Effects
	///
	/// * If the function is successful, it will mutate the state of the order and create a
	///   transaction.
	///
	/// # Panics
	///
	...
	/// # Side Effects
	///
	/// * If the function is successful, it will mutate the state of the transaction, setting the
	...
	/// # Note
	///
	/// * Before calling this function, make sure that the transaction_id exists and the caller is
	///   the seller.
		...
		<T as pallet_uniques::Config>::ItemId: From<u32>,
	{
		...
		// Ensure the transaction exists before trying to get it
			...
			<AfloatTransactions<T>>::contains_key(transaction_id),
	...
	/// * The specified offer does not exist.
	/// * The tax credit amount in the offer is less than the tax credit amount in the transaction
	///   (underflow).
	///
	/// # Side Effects
	///
	/// * If the function is successful, it will trigger the transfer of tax credits and Balance
	...
	/// # Note
	///
	/// * Before calling this function, make sure that the transaction id exists, the transaction is
	///   confirmed, and the caller is authorized.
	...
	fn role_id_to_afloat_role(role_id: RoleId) -> Option<AfloatRole> {
		AfloatRole::enum_to_vec()
			...
			.map(|role_bytes| {
				let role_str =
					str::from_utf8(role_bytes).expect("Role bytes should be valid UTF-8");
	...
	pub fn do_delete_all_users() -> DispatchResult {
		UserInfo::<T>::iter_keys().try_for_each(|account_id| {
			...
			if !is_admin_or_owner {
				...
				Self::remove_from_afloat_collection(
					account_id.clone(),
